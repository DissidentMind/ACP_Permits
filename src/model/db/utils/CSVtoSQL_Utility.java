...
public class CSVtoSQL_Utility {
    ...
    public CSVtoSQL_Utility(String PATH_FILE_CSV) throws Exception {
        ...
        for(int i = 0; i < listCommunications.size(); i++){
            if(getRegexValid.findCurrentIncidenteInString(listCommunications.get(i).toString(), DefaultsLoader.getRegexMetaTvdrOut()) != null){
                System.out.println("True => Letter Out: "+getRegexValid.findCurrentIncidenteInString(listCommunications.get(i).toString(), DefaultsLoader.getRegexMetaTvdrOut()));
                cOut++;
            }else{
                if(getRegexValid.findCurrentIncidenteInString(listCommunications.get(i).toString(), DefaultsLoader.getRegexMetaTvdrIn()) != null){
                    System.out.println("True => Letter IN: "+getRegexValid.findCurrentIncidenteInString(listCommunications.get(i).toString(), DefaultsLoader.getRegexMetaTvdrIn()));
                    cIn++;
                }else{
                    cOth++;
                    getOthersOrFails.add(listCommunications.get(i).toString()+"\n");
                }
            }
        }
